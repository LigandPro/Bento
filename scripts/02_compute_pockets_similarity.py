...
def prepare_binding_site(
    ...
    stem = os.path.splitext(os.path.basename(bs_path))[0].lower()
    if "pdb" in stem:
        print(
            f"Warning: glosa does not accept 'pdb' substring in filename, rename the file {bs_path}; 'PDB' is acceptable"
        )

    cut_bs(protein_path, ligand_path, bs_path, 4.5)  # 4.5 angstrom cutoff

    with open(bs_path, "r", encoding="utf-8", errors="ignore") as handle:
    ...
    filtered.append("TER\n")
    filtered.append("END\n")
    with open(bs_path, "w", encoding="utf-8") as handle:
...
def calculate_glosa_score(bs_path_1: str, bs_path_2: str, glosa_dir: str, timeout: float = 500):
    try:
        ...
        result = subprocess.run(
            command_list, shell=False, cwd=glosa_dir, capture_output=True, timeout=timeout
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        required=True,
        type=str,
        help="file to write calculcated glosa scores csv table",
    )
    parser.add_argument(
        "-data-csv", required=True, type=str, help="filename for pd.dataframe to read"
    )
    parser.add_argument(
        "-protein-path", required=True, type=str, help="column name for protein path"
    )
    parser.add_argument("-ligand-path", required=True, type=str, help="column name for ligand path")
    parser.add_argument(
        "-bs-dir", required=True, type=str, help="folder name to store binding sites"
    )
    parser.add_argument(
        "-bs-column", required=False, type=str, help="column name for binding site path"
    )
    parser.add_argument("-glosa-dir", required=True, type=str, help="folder with glosa programm")
    args = parser.parse_args()

    df = pd.read_csv(args.data_csv, sep=None, engine="python")
    print(f"Read {args.data_csv} with {len(df)} rows")
    if args.bs_column is None:
        df["bs_path"] = df[args.protein_path].apply(
            lambda x: os.path.join(args.bs_dir, os.path.basename(x))
        )
    else:
