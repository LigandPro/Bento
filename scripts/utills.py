...
def group_tools_pairwise_cliques(df, subset_ids, alpha=0.05, method="holm"):
    ...
    successes = np.array(
        [sub[tool].apply(lambda x: bool(x[1])).sum() for tool in tool_list], dtype=int
    )
...
def plot_grouped_bars(data, title, w=1500, h=500):
    ...
    for subset in subsets:
        ...
        if subset_tools:
            ...
            best_first[subset] = {
                t for t in subset_tools if abs(subset_dict[t][0] - max_first_val) <= tol
            }
            best_second[subset] = {
                t for t in subset_tools if abs(subset_dict[t][1] - max_second_val) <= tol
            }

    # Bars
    for x, f_val, s_val, subset, tool in zip(
        x_vals, first_vals, second_vals, subset_for_bar, tool_for_bar, strict=False
    ):
        base_color = "#8ba8b7"
        ...
        fig.add_trace(
            go.Bar(
                ...
                marker=dict(
                    color="rgba(0,0,0,0)",
                    line=dict(color=hatched_color, width=1.5),
                    pattern=dict(shape="/", fgcolor=hatched_color),
                ),
                showlegend=False,
            )
        )
    ...
    for x, f_val, s_val in zip(x_vals, first_vals, second_vals, strict=False):
    ...
    fig.update_layout(
        ...
        legend=dict(orientation="h", x=0.5, y=1.25, xanchor="center", yanchor="top"),
        width=w,
        height=h,
    ...
    for subset in subsets:
        subset_dict, subset_groups = data[subset]
        n_bars = sum(len([t for t in subgroup if t in subset_dict]) for subgroup in subset_groups)
        if n_bars == 0:
            continue
        start = cumulative
